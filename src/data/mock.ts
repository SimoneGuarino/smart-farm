...
export function buildMockSensors(): Sensor[] {
    ...
    [territories[0].id, territories[1].id].forEach(tid => {
        vwcRows.forEach((row) => {
            depths.forEach((d) => {
                const sector = sectors.find(s => s.rows.includes(row))!.id
                sensors.push({ id: `VWC_${row}_${d}`, territoryId: tid, kind: 'VWC', label: `VWC fila ${row} @${d}cm`, row, depthCm: d, sector, unit: '%v/v', battery: randBetween(65, 100) })
            })
        })

        // EC drenaggio/percolato – 2 sonde
        sensors.push({ id: 'ECDRAIN_A', territoryId: tid, kind: 'EC_DRAIN', label: 'EC drenaggio – testa impianto', sector: 'A', unit: 'mS/cm', battery: randBetween(60, 100) })
        sensors.push({ id: 'ECDRAIN_E', territoryId: tid, kind: 'EC_DRAIN', label: 'EC drenaggio – valle impianto', sector: 'E', unit: 'mS/cm', battery: randBetween(60, 100) })

        // Temperatura suolo dedicata – 1
        sensors.push({ id: 'T_SOIL_CENTRO', territoryId: tid, kind: 'SOIL_TEMP', label: 'T suolo 15cm – centro', sector: 'C', unit: '°C', battery: randBetween(70, 100) })

        // Leaf wetness – 2
        sensors.push({ id: 'LWS_1', territoryId: tid, kind: 'LEAF_WETNESS', label: 'Bagnatura fogliare – est', sector: 'B', unit: 'arb', battery: randBetween(50, 100) })
        sensors.push({ id: 'LWS_2', territoryId: tid, kind: 'LEAF_WETNESS', label: 'Bagnatura fogliare – ovest', sector: 'D', unit: 'arb', battery: randBetween(50, 100) })

        // Inline pH/EC (mandata)
        sensors.push({ id: 'PH_INLINE', territoryId: tid, kind: 'PH_INLINE', label: 'pH in linea', unit: 'pH' })
        sensors.push({ id: 'EC_INLINE', territoryId: tid, kind: 'EC_INLINE', label: 'EC in linea', unit: 'mS/cm' })
        ...
        sensors.push({ id: 'P_PRE', territoryId: tid, kind: 'PRESSURE', label: 'Pressione pre-filtro', unit: 'bar' })
        sensors.push({ id: 'P_POST', territoryId: tid, kind: 'PRESSURE', label: 'Pressione post-filtro', unit: 'bar' })

        // Stazione meteo aggregata
        sensors.push({ id: 'METEO', territoryId: tid, kind: 'METEO', label: 'Meteo (temp, UR, pioggia, vento)', unit: 'composite' })
    });
